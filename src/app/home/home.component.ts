...
export class HomeComponent implements OnInit {
  ...
  http: any;
  base64Image!: string;
  constructor(private processosService: ProcessosService, private sanitizer: DomSanitizer) { 
  ...
  buscarArquivoHtml(){
    this.processosService.buscarArquivoHtml(this.processo.numero,this.arquivoHtml).subscribe(data => {
      this.htmlSafe = this.sanitizer.bypassSecurityTrustHtml(data);  // acessa a propriedade Search
   }) 
  }
